...
inline std::filesystem::path search_path(
        ...
        const std::vector<std::filesystem::path> &path)
{
    ...
    for (const std::filesystem::path & pp_ : path)
    {
        auto p = pp_ / filename;
        for (std::filesystem::path ext : extensions)
        {
            std::filesystem::path pp = p;
            pp += ext;
            std::error_code ec;
            bool file = std::filesystem::is_regular_file(pp, ec);
            if (!ec && file &&
