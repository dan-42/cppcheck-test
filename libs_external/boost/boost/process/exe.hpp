...
struct exe_
{
    ...
    inline exe_setter_<typename std::filesystem::path::value_type> operator()(const std::filesystem::path & pth) const
    {
        return exe_setter_<typename std::filesystem::path::value_type>(pth.native());
    }
...
a `std::basic_string` with `char` or `wchar_t` or a `std::filesystem::path`.
