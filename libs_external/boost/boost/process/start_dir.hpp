...
struct start_dir_
{
    ...
    api::start_dir_init<typename std::filesystem::path::value_type>
                              operator()(const std::filesystem::path & st) const {return {st.native()}; }
...
It can be used with `std::string`, `std::wstring` and `std::filesystem::path`.
