...
#if BOOST_OS_WINDOWS
namespace detail {
    inline std::experimental::filesystem::path program_location_impl(boost::dll::fs::error_code& ec) {
    ...
    */
    template <class T>
    inline std::experimental::filesystem::path symbol_location_ptr(T ptr_to_symbol, boost::dll::fs::error_code& ec) {
        ...
        std::experimental::filesystem::path ret;
        if (!ptr_to_symbol) {
    ...
    template <class T>
    inline std::experimental::filesystem::path symbol_location_ptr(T ptr_to_symbol) {
        std::experimental::filesystem::path ret;
    ...
    */
    template <class T>
    inline std::experimental::filesystem::path symbol_location(const T& symbol, boost::dll::fs::error_code& ec) {
        ec.clear();
    ...
    template <class T>
    inline std::experimental::filesystem::path symbol_location(const T& symbol, const char* /*workaround*/ = 0)
#else
    //! \overload symbol_location(const T& symbol, boost::dll::fs::error_code& ec)
    template <class T>
    inline std::experimental::filesystem::path symbol_location(const T& symbol)
#endif
    {
        std::experimental::filesystem::path ret;
        boost::dll::fs::error_code ec;
    ...
    * \throws std::bad_alloc in case of insufficient memory. Overload that does not accept \forcedlinkfs{error_code} also throws \forcedlinkfs{system_error}.
    */
    static inline std::experimental::filesystem::path this_line_location(boost::dll::fs::error_code& ec) {
        typedef std::experimental::filesystem::path(func_t)(boost::dll::fs::error_code& );
        func_t& f = this_line_location;
        return boost::dll::symbol_location(f, ec);
    }

    //! \overload this_line_location(boost::dll::fs::error_code& ec)
    static inline std::experimental::filesystem::path this_line_location() {
        std::experimental::filesystem::path ret;
        boost::dll::fs::error_code ec;
    ...
    * \throws std::bad_alloc in case of insufficient memory. Overload that does not accept \forcedlinkfs{error_code} also throws \forcedlinkfs{system_error}.
    */
    inline std::experimental::filesystem::path program_location(boost::dll::fs::error_code& ec) {
        ec.clear();
        return boost::dll::detail::program_location_impl(ec);
    }

    //! \overload program_location(boost::dll::fs::error_code& ec) {
    inline std::experimental::filesystem::path program_location() {
        std::experimental::filesystem::path ret;
        boost::dll::fs::error_code ec;
