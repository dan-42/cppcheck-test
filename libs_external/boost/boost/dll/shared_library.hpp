...
public:
    ...
    * \throw \forcedlinkfs{system_error}, std::bad_alloc in case of insufficient memory.
    */
    explicit shared_library(const std::experimental::filesystem::path& lib_path, load_mode::type mode = load_mode::default_mode) {
    ...
    * \throw std::bad_alloc in case of insufficient memory.
    */
    shared_library(const std::experimental::filesystem::path& lib_path, boost::dll::fs::error_code& ec, load_mode::type mode = load_mode::default_mode) {
        shared_library::load(lib_path, mode, ec);
    }

    //! \overload shared_library(const std::experimental::filesystem::path& lib_path, boost::dll::fs::error_code& ec, load_mode::type mode = load_mode::default_mode)
    shared_library(const std::experimental::filesystem::path& lib_path, load_mode::type mode, boost::dll::fs::error_code& ec) {
        shared_library::load(lib_path, mode, ec);
    }
    ...
    */
    shared_library& assign(const shared_library& lib, boost::dll::fs::error_code& ec) {
        ...
        std::experimental::filesystem::path loc = lib.location(ec);
        if (ec) {
            return *this;
        }
    ...
    */
    ...
    //! \overload void load(const std::experimental::filesystem::path& lib_path, boost::dll::fs::error_code& ec, load_mode::type mode = load_mode::default_mode)
...
public:
    ...
    */
    std::experimental::filesystem::path location() const {
        boost::dll::fs::error_code ec;
        if (!is_loaded()) {
            ec = boost::dll::fs::make_error_code(
                boost::dll::fs::errc::bad_file_descriptor
        ...
        std::experimental::filesystem::path full_path = base_t::full_module_path(ec);
    ...
    * \throw std::bad_alloc.
    */
    std::experimental::filesystem::path location(boost::dll::fs::error_code& ec) const {
        if (!is_loaded()) {
            ec = boost::dll::fs::make_error_code(
                boost::dll::fs::errc::bad_file_descriptor
            );
    ...
    */
    static std::experimental::filesystem::path suffix() {
    ...
    */
    static std::experimental::filesystem::path decorate(const std::experimental::filesystem::path& sl) {
