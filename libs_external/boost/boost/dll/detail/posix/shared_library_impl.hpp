...
public:
    ...
    static std::experimental::filesystem::path decorate(const std::experimental::filesystem::path & sl) {
        std::experimental::filesystem::path actual_path = (
            std::strncmp(sl.filename().string().c_str(), "lib", 3)
            ? std::experimental::filesystem::path((sl.has_parent_path() ? sl.parent_path() / L"lib" : L"lib").native() + sl.filename().native())
            : sl
        );
        actual_path += suffix();
        return actual_path;
    ...
    void load(std::experimental::filesystem::path sl, load_mode::type mode, boost::dll::fs::error_code &ec) {
        ...
        if (!sl.is_absolute() && !(mode & load_mode::search_system_folders)) {
            ...
            std::experimental::filesystem::path prog_loc = std::experimental::filesystem::current_path(current_path_ec);
            if (!current_path_ec) {
                prog_loc /= sl;
                sl.swap(prog_loc);
            }
        ...
        if (!!(mode & load_mode::append_decorations)) {
            ...
            std::experimental::filesystem::path actual_path = decorate(sl);
            ...
            std::experimental::filesystem::path loc = boost::dll::detail::program_location_impl(prog_loc_err);
            if (std::experimental::filesystem::exists(actual_path) && !boost::dll::fs::equivalent(sl.string(), loc.string(), prog_loc_err)) {
        ...
        boost::dll::fs::error_code prog_loc_err;
        std::experimental::filesystem::path loc = boost::dll::detail::program_location_impl(prog_loc_err);
        if (!prog_loc_err && boost::dll::fs::equivalent(sl.string(), loc.string(), prog_loc_err) && !prog_loc_err) {
    ...
    std::experimental::filesystem::path full_module_path(boost::dll::fs::error_code &ec) const {
