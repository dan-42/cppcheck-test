...
public:
    ...
    static std::experimental::filesystem::path decorate(const std::experimental::filesystem::path& sl) {
        std::experimental::filesystem::path actual_path = sl;
        actual_path += suffix();
        return actual_path;
    }
    ...
    void load(std::experimental::filesystem::path sl, load_mode::type mode, boost::dll::fs::error_code &ec) {
        ...
        if (!!(mode & load_mode::append_decorations)) {
            ...
            const std::experimental::filesystem::path mingw_load_path = (
                sl.has_parent_path()
...
private:
    ...
    bool load_impl(const std::experimental::filesystem::path &load_path, boost::winapi::DWORD_ mode, boost::dll::fs::error_code &ec) {
        ...
        ec = boost::dll::detail::last_error_code();
        if (std::experimental::filesystem::exists(load_path)) {
